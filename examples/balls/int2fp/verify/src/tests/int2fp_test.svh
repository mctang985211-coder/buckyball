class int2fp_ball_test extends uvm_test;
  ...
  task run_phase(uvm_phase phase);
    int unsigned bid = int2fp_require_bid();
    phase.raise_objection(this);
    for (int unsigned index = 0; index < INT2FP_NUM_CASES; index++) run_case(index, bid);
  ...
  task run_case(int unsigned index, int unsigned bid);
    ...
    while (!env.scb.done()) begin
      ...
      if (cycles > INT2FP_TIMEOUT_CYCLES)
        `uvm_fatal("TIMEOUT", $sformatf("Int2FpBall case %0d timeout", index))
    end
    `uvm_info("INT2FP", $sformatf("Int2FpBall case %0d passed in %0d cycles", index, cycles),
              UVM_LOW)
  endtask
