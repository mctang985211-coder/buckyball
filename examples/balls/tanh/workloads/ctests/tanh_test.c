...
/* Golden tanh: y = tanh(x) in the exact form (reference: torch.tanh).
 * Evaluated in IEEE double with a 9-term Taylor exp and an exact 2^k
 * bit-scaled exponent (no libm), mirroring tanh_gold() in
 * examples/balls/tanh/emu/src/58_tanh.rs term for term: with
 * e = exp(-2|x|) (|x| clamped to 100), tanh(x) = sign(x) * (1-e)/(1+e). */
