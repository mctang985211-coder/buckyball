//===- 59_silu.rs - SILU instruction (exact fp32 SiLU) ---------------------===//
//
// y = x * sigmoid(x) = x / (1 + e^{-x}), the exact form (reference:
// torch.nn.functional.silu / transformers LlamaMLP hidden_act silu, never a
// piecewise or rational approximation). fp32 in / fp32 out.
//
...
//
// A bank row is 16 bytes = 4 fp32 lanes (pebble bank_width = 128). The ball
// reads input rows 0..n/4-1 and writes silu() of every lane to the same
...
// sequence as `silu_ref()` in workloads/ctests/silu_test.c and the mlir_tests
// check_result. The ctests compare with |got - expected| <= 1e-4f, which
// bounds the RTL contract.
//
...
/// exp(y) for y in [-100, 0], pure bit arithmetic (no libm, FMA-free): a
/// 9-term Taylor series in the range-reduced mantissa times an exact 2^k
/// exponent add. Term-for-term identical to `silu_exp_neg()` in
/// workloads/ctests/silu_test.c and in the mlir_tests check_result.
...
fn silu_gold(x: f32) -> f32 {
    let xd = f64::from(x);
    let a = if xd < 0.0 { -xd } else { xd };
    // exp(-a) saturates the fp32 result: a >= 100 gives e <= 3.7e-44,
    // well below one fp32 ulp of every affected output.
...
fn decode_validate(xs1: u64, xs2: u64) -> (usize, usize, usize) {
    if xs2 != 0 {
        panic!("silu: reserved xs2 must be zero, got {xs2}");
    }
    if rs1_b1(xs1) != 0 {
        panic!("silu: reserved rs1 BANK1 field must be zero");
    }
    let n = rs1_iter(xs1);
    if n == 0 {
        panic!("silu: N must be positive");
    }
    if n % 4 != 0 {
        panic!("silu: N must be a multiple of 4 (16B bank row), got {n}");
    }
    if n / 4 > bank_lines() as u64 {
        panic!("silu: N = {n} exceeds bank capacity (4 * {} lines)", bank_lines());
    }
    let in_bank = rs1_b0(xs1);
    let out_bank = rs1_b2(xs1);
    if in_bank >= bank_num() as u64 {
        panic!("silu: invalid in_bank id {in_bank}");
    }
    if out_bank >= bank_num() as u64 {
        panic!("silu: invalid out_bank id {out_bank}");
    }
    if in_bank == out_bank {
        panic!("silu: in_bank and out_bank must be distinct, both {in_bank}");
    }
    (in_bank as usize, out_bank as usize, (n / 4) as usize)
}

impl BallInstruction for Silu {
    fn exec(xs1: u64, xs2: u64, ctx: &mut ExecContext) -> u64 {
        let (in_bank, out_bank, rows) = decode_validate(xs1, xs2);
        for bank in [in_bank, out_bank] {
            let config = ctx.cfgs[bank];
            if !config.allocated {
                panic!("silu: bank {bank} not allocated");
            }
            if config.cols != 1 {
                panic!("silu: bank {bank} must be a single-group bank, cols = {}", config.cols);
            }
        }
        ...
        for row in 0..rows {
            ...
            for lane in 0..4 {
                ...
                write[off..off + 4].copy_from_slice(&silu_gold(x).to_le_bytes());
            }
        }
        0
    }
