...
// bb_silu(in_bank, out_bank, n): fp32 y = x * sigmoid(x) = x / (1 + e^{-x})
// (reference: torch.nn.functional.silu; SwiGLU gate activation). Elementwise
// 1rd+1wr: one single-group input bank, one distinct single-group output
...
// examples/balls/silu/emu/src/59_silu.rs; the C golden is silu_ref() in
// examples/balls/silu/workloads/ctests/silu_test.c.
#define bb_silu(in_bank, out_bank, n)                                          \
