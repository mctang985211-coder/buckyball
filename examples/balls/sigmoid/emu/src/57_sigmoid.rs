//===- 57_sigmoid.rs - SIGMOID instruction (exact fp32 sigmoid) -----------===//
//
// y = 1 / (1 + exp(-x)), the exact form (reference: torch.sigmoid, never a
// piecewise-linear or rational approximation). fp32 in / fp32 out.
//
...
// reads input rows 0..n/4-1 and writes sigmoid() of every lane to the same
...
// sequence as `sigmoid_ref()` in workloads/ctests/. The ctests compare with
// |got - expected| <= 1e-4f, which bounds the RTL contract.
//
...
/// exponent add. Term-for-term identical to `sigmoid_exp_neg()` in
/// workloads/ctests/sigmoid_test.c and to the model in /tmp validation.
...
fn decode_validate(xs1: u64, xs2: u64) -> (usize, usize, usize) {
    if xs2 != 0 {
        panic!("sigmoid: reserved xs2 must be zero, got {xs2}");
    }
    if rs1_b1(xs1) != 0 {
        panic!("sigmoid: reserved rs1 BANK1 field must be zero");
    }
    let n = rs1_iter(xs1);
    if n == 0 {
        panic!("sigmoid: N must be positive");
    }
    if n % 4 != 0 {
        panic!("sigmoid: N must be a multiple of 4 (16B bank row), got {n}");
    }
    if n / 4 > bank_lines() as u64 {
        panic!("sigmoid: N = {n} exceeds bank capacity (4 * {} lines)", bank_lines());
    }
    let in_bank = rs1_b0(xs1);
    let out_bank = rs1_b2(xs1);
    if in_bank >= bank_num() as u64 {
        panic!("sigmoid: invalid in_bank id {in_bank}");
    }
    if out_bank >= bank_num() as u64 {
        panic!("sigmoid: invalid out_bank id {out_bank}");
    }
    if in_bank == out_bank {
        panic!("sigmoid: in_bank and out_bank must be distinct, both {in_bank}");
    }
    (in_bank as usize, out_bank as usize, (n / 4) as usize)
}

impl BallInstruction for Sigmoid {
    fn exec(xs1: u64, xs2: u64, ctx: &mut ExecContext) -> u64 {
        let (in_bank, out_bank, rows) = decode_validate(xs1, xs2);
        for bank in [in_bank, out_bank] {
            let config = ctx.cfgs[bank];
            if !config.allocated {
                panic!("sigmoid: bank {bank} not allocated");
            }
            if config.cols != 1 {
                panic!("sigmoid: bank {bank} must be a single-group bank, cols = {}", config.cols);
            }
        }
        ...
        for row in 0..rows {
            ...
            for lane in 0..4 {
                ...
                write[off..off + 4].copy_from_slice(&sigmoid_gold(x).to_le_bytes());
            }
        }
        0
    }
