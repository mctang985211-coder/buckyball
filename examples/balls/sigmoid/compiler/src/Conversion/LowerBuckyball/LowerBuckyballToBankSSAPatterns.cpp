//===- LowerBuckyballToBankSSAPatterns.cpp - SigmoidBall bank-SSA lowering
//-===//
//
// Lowers the logical FP32 sigmoid tile to the bank-SSA chain
// alloc -> mvin -> bank_sigmoid -> mvout -> fence -> release.  Like the GELU
// precedent this file keeps its bank helpers local instead of including
// "Utils/BankUtils.h": that header instantiates createBankSMatMul, which
// needs the SMatMul dialect ops that are only generated for unions mounting
// SMatMulBall.  SigmoidBall lives on Toy, whose union does not have them.
//
//===----------------------------------------------------------------------===//
...
void populateSigmoidBallLowerBuckyballToBankSSAPatterns(
...
class SigmoidMatrixToBankSSAPattern : public OpRewritePattern<SigmoidMatrixOp> {
public:
  using OpRewritePattern<SigmoidMatrixOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(SigmoidMatrixOp op,
    ...
    if (target.bankWidthBits != 128 || target.bankDepth <= 0 ||
        target.bankDepth % 16)
      return op.emitError(
          "SigmoidBall requires a 128-bit bank with 16-aligned depth");

    Location loc = op.getLoc();
    int64_t lines = target.bankDepth;
    int64_t values = lines * kValuesPerLine;
    int64_t total = rows * columns;
    // Sigmoid is 1rd+1wr with distinct banks: the input bank receives the
    ...
    for (int64_t base = 0; base < total; base += values) {
      ...
      // Sigmoid element count (fp32 lanes), not row count: the ball contract
      // in examples/balls/sigmoid/emu/src/57_sigmoid.rs.
      Value result =
          b.create<BankSigmoidOp>(loc, loaded.getType(), loaded, outBank,
...
void mlir::buddy::populateSigmoidBallLowerBuckyballToBankSSAPatterns(
    RewritePatternSet &patterns) {
  patterns.add<SigmoidMatrixToBankSSAPattern>(patterns.getContext());
}
