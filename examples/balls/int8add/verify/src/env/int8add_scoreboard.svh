class int8add_scoreboard extends uvm_scoreboard;
  `uvm_component_utils(int8add_scoreboard)

  uvm_analysis_imp_stim #(bb_blink_cmd_item, int8add_scoreboard) stim_imp;
  uvm_analysis_imp_cmd #(bb_blink_cmd_item, int8add_scoreboard) cmd_imp;
  uvm_analysis_imp_read #(bb_blink_read_item, int8add_scoreboard) read_imp;
  uvm_analysis_imp_write #(bb_blink_write_item, int8add_scoreboard) write_imp;
  uvm_analysis_imp_resp #(bb_blink_resp_item, int8add_scoreboard) resp_imp;

  bb_blink_mem_model #(`BB_IN_BW, `BB_OUT_BW) mem_model;

  int8add_cmd_item stim_q[$];
  ...
  function void write_stim(bb_blink_cmd_item item);
    int8add_cmd_item got;
    int8add_cmd_item clone;
    int i;
    ...
    if (!$cast(got, item)) begin
      `uvm_fatal("SCB", "stim item is not int8add_cmd_item")
    end
    ...
    mem_model.clear_mem();
    for (i = 0; i < clone.num_lhs_words; i++) begin
      mem_model.load_word(int'(clone.op1_bank), i, clone.lhs_words[i]);
    end
    for (i = 0; i < clone.num_rhs_words; i++) begin
      mem_model.load_word(int'(clone.op2_bank), i, clone.rhs_words[i]);
    end
    mem_model.arm();
  ...
  function void write_cmd(bb_blink_cmd_item item);
    int8add_cmd_item stim;
    stim = current_stim("CMD");
    check_cmd(item, stim);
    cmd_count++;
  endfunction

  function void check_cmd(bb_blink_cmd_item got, int8add_cmd_item exp);
    if (got.bid !== exp.bid)
  ...
  function void write_read(bb_blink_read_item item);
    ...
    if (item.port != 0 && item.port != 1)
      `uvm_fatal("READ", $sformatf("Int8AddBall has two read ports, got port %0d", item.port))
  ...
  function void write_write(bb_blink_write_item item);
    ...
    if (item.port != 0)
      `uvm_fatal("WRITE", $sformatf("Int8AddBall has one write port, got port %0d", item.port))
  ...
  function int8add_cmd_item current_stim(string tag);
    if (stim_q.size() == 0) begin
