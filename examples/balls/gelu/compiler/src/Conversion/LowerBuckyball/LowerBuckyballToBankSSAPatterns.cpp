//===- LowerBuckyballToBankSSAPatterns.cpp - GeluBall bank-SSA lowering ---===//
//
// Lowers the logical FP32 GELU tile to the bank-SSA chain
// alloc -> mvin -> bank_gelu -> mvout -> fence -> release.  Unlike the ReLU
...
// SMatMulBall.  GeluBall lives on Toy, whose union does not have them.
//
//===----------------------------------------------------------------------===//
...
void populateGeluBallLowerBuckyballToBankSSAPatterns(
...
class GeluMatrixToBankSSAPattern : public OpRewritePattern<GeluMatrixOp> {
public:
  using OpRewritePattern<GeluMatrixOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(GeluMatrixOp op,
    ...
    if (target.bankWidthBits != 128 || target.bankDepth <= 0 ||
        target.bankDepth % 16)
      return op.emitError(
          "GeluBall requires a 128-bit bank with 16-aligned depth");

    Location loc = op.getLoc();
    int64_t lines = target.bankDepth;
    int64_t values = lines * kValuesPerLine;
    int64_t total = rows * columns;
    // GELU is 1rd+1wr with distinct banks: the input bank receives the
    ...
    for (int64_t base = 0; base < total; base += values) {
      ...
      // GELU element count (fp32 lanes), not row count: the ball contract
      // in examples/balls/gelu/emu/src/56_gelu.rs.
      Value result =
          b.create<BankGeluOp>(loc, loaded.getType(), loaded, outBank,
...
void mlir::buddy::populateGeluBallLowerBuckyballToBankSSAPatterns(
    RewritePatternSet &patterns) {
  patterns.add<GeluMatrixToBankSSAPattern>(patterns.getContext());
}
